...
const increasePage = async () => {
  if (page.value < pagination.value?.totalPages) {
    ...
    toast.info("Đã tải hết từ vựng");
  }
};

const handleShowDetail = (vocabulary) => {
  vocabularyDetail.value = vocabulary;
  showDetail.value = true;
};

const getVocabularies = async () => {
  try {
    if (!user.value?.topic_id) {
      vocabularies.value = [];
      return null;
    } else {
      ...
      vocabularies.value = vocabularies.value.concat(data?.vocabularies || []);
...
<template>
  ...
  />
  <div v-else id="vocabulary" class="flex">
    <Sidebar />
    <div class="vocabulary-main w-full flex-1">
      <Header :user="user" />
      <div class="vocabulary-content mx-auto">
        <div class="vocabulary-content__header flex flex-col items-center gap-5">
        ...
        <template v-else>
          <div class="vocabulary-content__body mt-6">
              ...
              <div class="dropdown dropdown-bottom dropdown-end">
                ...
                >
                  <li :class="{ active: sort === 'createdAt:desc' }">
                    <a @click.prevent="handleFilter('createdAt:desc')"
                      >Mới thêm gần đây</a
                    >
                  </li>
            ...
            <div class="list-vocabulary mt-6">
              <div
                v-for="vocabulary in vocabularies"
                :key="vocabulary?._id"
                class="flex items-center gap-10 p-8 bg-white item"
              >
                <button
                  class="flex items-center justify-center rounded-2xl w-[48px] h-[48px] bg-[#f6f6f6] hover:scale-110"
                  @click.prevent="
                    playAudio(URL_API, vocabulary?.audio?.uk, vocabulary?.word)
                  "
                >
                  <svg
                    width="44"
                ...
                <div class="flex flex-col gap-5 text-[1.8rem]">
                  <span class="font-bold">{{ vocabulary?.word }}</span>
                  <span>{{ vocabulary?.vietnamese }}</span>
                </div>
                <div class="ms-auto">
                  <button class="py-4" @click="handleShowDetail(vocabulary)">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
              ...
              <button
                ...
                class="flex items-center gap-10 justify-between w-full font-bold p-8 bg-white item text-[1.8rem]"
                @click="increasePage"
              >
                <span>Tải thêm</span>
                <span>
      ...
      <div class="modal-header flex items-center justify-between mb-5">
        <div class="flex flex-col gap-8 w-full">
          <div class="relative flex items-center justify-center gap-3">
            <h3 class="text-[2.5rem] font-bold">Chi tiết từ vựng</h3>
            <span
              class="absolute right-0 icon cursor-pointer"
              @click="showDetail = false"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          <div class="sm:grid sm:grid-cols-2 gap-5">
            ...
            <div class="flex col-span-2 flex-col md:flex-row gap-5 mt-5 text-[1.8rem]">
              ...
              <div class="flex flex-col gap-3">
                <span
                  v-for="example in vocabularyDetail?.examples"
                  :key="example"
                  class="badge bg-secondary h-full"
                  style="
                    text-align: left;
                    font-size: 1.8rem;
                    padding: 8px;
